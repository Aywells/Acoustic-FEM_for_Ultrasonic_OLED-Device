# -*- coding: utf-8 -*-
"""
Controlled OLED design with selective acoustical analysis; switchs and variables able to control signal and specific computations
"""
...
TargFileB = '1.flxhst' # phase- and scale-shifted, 'target' file
TransmitB = '3.flxhst'
...
graph_title = 'RX Electrode Current: Phase Shift Amplitude Comparison'

max_time = 6e-6 #second peak ends near .192e-6
elec = 1
Targ_A_reflection_start = 200 #ns
Targ_A_reflection_end = 750#400
...
Targ_A_minus_transmit_reflect = [x[Targ_A_reflection_start_index:Targ_A_reflection_end_index] for x in Targ_A_minus_transmit]
Targ_B_minus_transmit_reflect_unphased = [x[Targ_A_reflection_start_index:Targ_A_reflection_end_index] for x in Targ_B_minus_transmit] #not phase shifted
...
if USE_PHASE_OPTIMIZATION == True:
    ...
    for electrode in range(len(TarglistA)):
        shift0 = match_maxima_v4(Targ_A_minus_transmit_reflect[electrode], Targ_B_minus_transmit_reflect_unphased[electrode])
    ...
    Targ_B_minus_transmit_reflect = [x[Targ_B_reflection_start_index:Targ_B_reflection_end_index] for x in Targ_B_minus_transmit]
...
def compare_reflection(scale, untarg, targ): #takes two signal lists and a scalar, and returns a comparative value of the error between the two. 
    scaled_targ = [x*scale for x in targ]
    difference = [abs(x[1] - x[0]) for x in list(zip(untarg, scaled_targ))]  #absolute value measures total error, not equated pos/neg error
...
if USE_SCALE_OPTIMIZATION == True:
    ...
    for electrode in range(len(TarglistA)):
        ...
        for scale_est in test_range:
            diff_list.append(compare_reflection(scale_est, Targ_A_minus_transmit_reflect[electrode], Targ_B_minus_transmit_reflect[electrode]))
...
if PLOT_SIGNAL == True:
    ...
    plt.plot(Targ_B_reflection_times, scaled_Targ_B_minus_transmit_reflect[elec], label = labelB + '; Electrode ' + str(elec))
